...
# Create a temp package to make relative imports work
os.makedirs('/home/claude/saaam_pkg', exist_ok=True)

# Copy project files to package
import shutil
for f in ['tokens.py', 'ast_nodes.py']:
    shutil.copy(f'/mnt/project/{f}', f'/home/claude/saaam_pkg/{f}')

# Copy lexer and fix relative imports
with open('/mnt/project/lexer.py', 'r') as f:
    lexer_content = f.read()
lexer_content = lexer_content.replace('from .tokens import', 'from saaam_pkg.tokens import')
with open('/home/claude/saaam_pkg/lexer.py', 'w') as f:
    f.write(lexer_content)

# Copy ast_nodes and fix relative imports
with open('/mnt/project/ast_nodes.py', 'r') as f:
    ast_content = f.read()
ast_content = ast_content.replace('from .tokens import', 'from saaam_pkg.tokens import')
with open('/home/claude/saaam_pkg/ast_nodes.py', 'w') as f:
    f.write(ast_content)

# Copy parser and fix imports
with open('/home/claude/parser.py', 'r') as f:
    parser_content = f.read()
parser_content = parser_content.replace('from .tokens import', 'from saaam_pkg.tokens import')
parser_content = parser_content.replace('from .lexer import', 'from saaam_pkg.lexer import')
parser_content = parser_content.replace('from .ast_nodes import', 'from saaam_pkg.ast_nodes import')
parser_content = parser_content.replace('from tokens import', 'from saaam_pkg.tokens import')
parser_content = parser_content.replace('from lexer import', 'from saaam_pkg.lexer import')
parser_content = parser_content.replace('from ast_nodes import', 'from saaam_pkg.ast_nodes import')
with open('/home/claude/saaam_pkg/parser.py', 'w') as f:
    f.write(parser_content)

# Create __init__.py
with open('/home/claude/saaam_pkg/__init__.py', 'w') as f:
    f.write('# SAAAM Language Package\n')

sys.path.insert(0, '/home/claude')

from saaam_pkg.tokens import TokenType
from saaam_pkg.lexer import Lexer, tokenize
from saaam_pkg.ast_nodes import *
from saaam_pkg.parser import Parser, parse, parse_expression_only, print_ast, format_expression, validate_syntax, is_complete_input
